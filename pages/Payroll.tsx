...
export const Payroll: React.FC = () => {
  ...
  const calculatePayroll = () => {
    ...
    // Simulate API delay
    setTimeout(() => {
      const payrollResults = MOCK_EMPLOYEES.map(emp => 
        PayrollCalculator.calculateNetPay(emp, MOCK_EMPLOYEES.length)
      );
  ...
  const handleDownloadPayslip = (res: PayrollResult) => {
    ...
    alert(`Generating official PDF payslip for ${res.employeeName} (October 2025).\n\nThis document includes:\n• Taxable Income Breakdown\n• Statutory Deductions (NSSF, PAYE, HESLB)\n• Employer Contributions\n• Compliance Watermark`);
  };
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-slate-400 text-sm font-medium">Ends in 5 days</span>
          </div>
          
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div>
              <p className="text-slate-400 font-medium mb-2">Estimated Net Disbursement</p>
              <h1 className="text-5xl font-bold tracking-tight">
                TZS {totalNetPay > 0 ? totalNetPay.toLocaleString() : '--'}
              </h1>
            </div>
            <div className="flex flex-wrap gap-4">
              ...
              {results.length > 0 && (
                <button className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-4 rounded-2xl font-bold transition-all shadow-xl shadow-emerald-500/20">
                  <Send size={20} />
                  Submit to TRA
                </button>
              )}
      ...
      {results.length > 0 ? (
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button className="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-700 rounded-xl text-xs font-bold hover:bg-slate-100 transition-all border border-slate-200">
                    <Download size={16} />
                    Download All
                  </button>
                </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-2 border-b border-slate-50">
                  ...
                  <span className="text-sm font-bold text-slate-900">TZS {results.reduce((s, r) => s + r.grossPay, 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-50">
                  <span className="text-sm text-slate-500 font-medium">NSSF (Employee 10%)</span>
                  <span className="text-sm font-bold text-slate-900">-TZS {results.reduce((s, r) => s + r.nssfEmployee, 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-50">
                ...
                <div className="flex justify-between items-center py-2 border-b border-slate-50">
                  ...
                  <span className="text-sm font-bold text-slate-900">-TZS {results.reduce((s, r) => s + r.heslbDeduction, 0).toLocaleString()}</span>
                </div>
                <div className="pt-4 flex justify-between items-center">
                  <span className="text-base font-bold text-slate-900">Net Disbursement</span>
                  <span className="text-xl font-black text-blue-600">TZS {totalNetPay.toLocaleString()}</span>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] p-20 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-slate-500 max-w-sm mb-8">
            Click the button above to start the calculation engine for the October 2025 cycle.
          </p>
